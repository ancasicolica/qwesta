function getHaxis(a) {
  var b, c;
  a.length < 25 ? (b = 6, c = 3) : a.length < 32 ? (b = 6, c = 6) : (b = 7, c = 0);
  var d = {gridlines: {count: b}, minorGridlines: {count: c}};
  return d
}
var initCharts = function () {
}, convertMySqlTimeToDate = function (a) {
  try {
    var b = a.split(" "), c = b[0].split("-"), d = b[1].split(":");
    return new Date(parseInt(c[0]), parseInt(c[1]) - 1, parseInt(c[2]), parseInt(d[0]), parseInt(d[1]), parseInt(d[2]))
  } catch (e) {
    return console.error("Invalid MySQL Date:" + a), new Date(2e3, 1, 1)
  }
}, drawTemperatureChart = function (a) {
  if (google) {
    var b = a;
    google.load("visualization", "1.0", {
      packages: ["corechart"], callback: function () {
        var a = new google.visualization.DataTable;
        a.addColumn("datetime", "Zeit"), a.addColumn("number", "Minimum"), a.addColumn("number", "Durchschnitt"), a.addColumn("number", "Maximum");
        for (var c = -100, d = 100, e = 0; e < b.length; e++) {
          var f = parseFloat(b[e].temperatureMin), g = parseFloat(b[e].temperatureMax);
          a.addRow([convertMySqlTimeToDate(b[e].tsLocal), f, parseFloat(b[e].temperatureAvg), g]), d > f && (d = f), g > c && (c = g)
        }
        var h, i;
        i = 0 > c ? 0 : 5 * (Math.floor(c / 5) + 1), h = d > 0 ? 0 : -5 * (Math.floor(Math.abs(d) / 5) + 1);
        var j = (i + Math.abs(h)) / 5 + 1, k = {
          curveType: "function",
          legend: {position: "bottom"},
          title: "Temperatur [°C]",
          vAxis: {minValue: h, maxValue: i, gridlines: {count: j}, minorGridlines: {count: 5}},
          hAxis: getHaxis(b)
        };
        console.log(screen.width);
        var l = new google.visualization.LineChart(document.getElementById("chart"));
        l.draw(a, k), $("#disclaimer").html("")
      }
    })
  }
}, drawHumidityChart = function (a) {
  if (google) {
    var b = a;
    google.load("visualization", "1.0", {
      packages: ["corechart"], callback: function () {
        var a = new google.visualization.DataTable;
        a.addColumn("datetime", "Zeit"), a.addColumn("number", "Luftfeuchtigkeit in %");
        for (var c = 0; c < b.length; c++)a.addRow([convertMySqlTimeToDate(b[c].tsLocal), parseFloat(b[c].humidityAvg)]);
        var d = {
          title: "Luftfeuchtigkeit [%]",
          curveType: "function",
          legend: {position: "bottom"},
          vAxis: {minValue: 0, maxValue: 100, gridlines: {count: 6}, minorGridlines: {count: 1}},
          hAxis: getHaxis(b)
        }, e = new google.visualization.LineChart(document.getElementById("chart"));
        e.draw(a, d), $("#disclaimer").html("")
      }
    })
  }
}, drawWindChart = function (a) {
  if (google) {
    var b = a;
    google.load("visualization", "1.0", {
      packages: ["corechart"], callback: function () {
        var a = new google.visualization.DataTable;
        a.addColumn("datetime", "Zeit"), a.addColumn("number", "Durchschnitt"), a.addColumn("number", "Maximum");
        for (var c = 0; c < b.length; c++)a.addRow([convertMySqlTimeToDate(b[c].tsLocal), parseFloat(b[c].windAvg), parseFloat(b[c].windMax)]);
        var d = {
          title: "Wind [km/h]",
          curveType: "function",
          legend: {position: "bottom"},
          hAxis: getHaxis(b)
        }, e = new google.visualization.AreaChart(document.getElementById("chart"));
        e.draw(a, d);
        var f = convertMySqlTimeToDate(b[0].tsLocal);
        (f.getMonth() < 3 || f.getMonth() > 9) && $("#disclaimer").html('Bei Schneefall ist es möglich, dass der Windsensor ausfällt. <a href="out-of-order.html">Mehr Infos hier.</a>')
      }
    })
  }
}, drawRainChart = function (a) {
  if (google) {
    var b = a, c = 1;
    google.load("visualization", "1.0", {
      packages: ["corechart"], callback: function () {
        var a = new google.visualization.DataTable;
        a.addColumn("datetime", "Zeit"), a.addColumn("number", "Niederschlag");
        for (var d = 0; d < b.length; d++) {
          var e = 3 * parseInt(b[d].raindif) / 10;
          a.addRow([convertMySqlTimeToDate(b[d].tsLocal), e]), e > c && (c = e)
        }
        var f = {
          ...
          vAxis: {minValue: 0, maxValue: c},
          hAxis: getHaxis(b)
        }, g = new google.visualization.ColumnChart(document.getElementById("chart"));
        g.draw(a, f), $("#disclaimer").html('Bei Schneefall werden keine Niederschlagsdaten erfasst. <a href="out-of-order.html">Mehr Infos hier.</a>')
      }
    })
  }
};
...
qwesta.controller("QwestaCtrl", ["$scope", "$http", "$interval", function (a, b) {
  var c = "http://www.kusti.ch/qwesta/qwestadata.php";
  initCharts(), a.initDate = (new Date).toDateString(), a.currentGraph = 0, a.startDate = Date.today(), a.minDate = Date.today(), a.maxDate = Date.today(), a.lastSetDate = a.startDate, a.format = "dd.MM.yyyy", a.data = {
    temperature: 0,
    humidity: 0,
    wind: 0,
    rain: 0,
    ...
    a.lastSetDate = a.startDate, a.set(a.currentGraph)
  };
  var d = function () {
    return "&day=" + a.startDate.getDate() + "&month=" + (a.startDate.getMonth() + 1) + "&year=" + a.startDate.getFullYear()
  }, e = function (b) {
    Date.today().addDays(b).compareTo(a.startDate) < 0 ? a.startDate = Date.today().addDays(b) : Date.today().addDays(b).compareTo(a.lastSetDate) >= 0 ? a.startDate = a.lastSetDate : Date.today().addDays(b).compareTo(a.startDate) >= 0 && (a.startDate = Date.today().addDays(b))
  };
  a.set = function (b) {
    a.currentGraph = b;
    for (var f = "?view=", g = null, h = 0; 12 > h; h++)$("#graph" + h).removeClass("qwesta-stats-selected");
    switch ($("#graph" + b).addClass("qwesta-stats-selected"), b) {
      case 0:
        e(0), f += "multi&range=day&temperature" + d(), g = drawTemperatureChart;
        break;
      case 1:
        e(-6), f += "multi&range=week&temperature" + d(), g = drawTemperatureChart;
        break;
      case 2:
        e(-29), f += "multi&range=month&temperature" + d(), g = drawTemperatureChart;
        break;
      case 3:
        e(0), f += "multi&range=day&humidity&day=" + d(), g = drawHumidityChart;
        break;
      case 4:
        e(-6), f += "multi&range=week&humidity&day=" + d(), g = drawHumidityChart;
        break;
      case 5:
        e(-29), f += "multi&range=month&humidity&day=" + d(), g = drawHumidityChart;
        break;
      case 6:
        e(0), f += "multi&range=day&wind&day=" + d(), g = drawWindChart;
        break;
      case 7:
        e(-6), f += "multi&range=week&wind&day=" + d(), g = drawWindChart;
        break;
      case 8:
        e(-29), f += "multi&range=month&wind&day=" + d(), g = drawWindChart;
        break;
      case 9:
        e(0), f += "multi&range=day&rain&day=" + d(), g = drawRainChart;
        break;
      case 10:
        e(-6), f += "multi&range=week&rain&day=" + d(), g = drawRainChart;
        break;
      case 11:
        e(-29), f += "multi&range=month&rain&day=" + d(), g = drawRainChart;
        break;
      default:
        return
    }
    var i = c + f;
    a.getCurrentData(i, function (a) {
      g(a.data)
    })
  }, a.getCurrentValues = function () {
    a.getCurrentData(c + "?view=current", function (b) {
      console.log(b), b.success ? (a.data.temperature = b.data[0].temperature, a.data.humidity = b.data[0].humidity, a.data.wind = b.data[0].wind, a.data.rain = 3 * b.data[0].raindifference / 10) : (a.data.temperature = 0, a.data.humidity = 0, a.data.wind = 0, a.data.rain = 0)
    })
  }, a.getCurrentData = function (a, c) {
    console.log("URL:" + a);
    var d = b.get(a, null);
  ...
  }, a.getCurrentValues(), a.set(0), a.getCurrentData(c + "?view=basics", function (b) {
    b.success && (a.minDate = convertMySqlTimeToDate(b.oldest), a.maxDate = convertMySqlTimeToDate(b.latest)), console.log(b)
  }), window.addEventListener("resize", a.setDate)
}]);
